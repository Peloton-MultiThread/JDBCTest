...
/*    stmt.execute(CREATE_STOCK_TABLE);
  ...
  public void Batch_Insert(int start, int numOfItems, int batchSize) throws Exception {
    PreparedStatement stmt = conn.prepareStatement(TEMPLATE_FOR_BATCH_INSERT);

    conn.setAutoCommit(false);
    
    for(int i = 1; i <= numOfItems ;i++){
      stmt.setInt(1,i + start);
      stmt.setString(2,"Yo");
      stmt.addBatch();
      if (i % batchSize == 0) {
        int[] res;
        ...
        for(int j=0; j < res.length; j++){
          ...
          if (res[j] < 0) {
            throw new SQLException("Query "+ (j+1) + (batchSize * i) +" returned " + res[i]);
          }
        }
  ...
  static public void main(String[] args) throws Exception {
      ...
      } else if (args[0].equals("libEvent")) {
        LibEventTest();
      } else if (args[0].equals("multiThread")) {
        MultiThreadTest();
      }
  }
  ...
  static public void LibEventTest() throws Exception{
    PelotonTest pt = new PelotonTest();
    pt.Init();
    System.out.println("Start inserting!!!!!!!!");
    long start = System.currentTimeMillis();
    pt.Batch_Insert(0, 10000, 10000);
    long end = System.currentTimeMillis();
    System.out.println("" + (end - start));
    pt.Close();
  }

  static public void MultiThreadTest() throws Exception {
    PelotonTest pt = new PelotonTest();
    pt.Init();
    pt.Close();
    ...
    for (int i = 0; i < 4 ; i++) {
      Thread t = new Thread(new MultiRunnable(i + 1));
      t.start();
    }
  }
  ...
  static class MultiRunnable implements Runnable {
    ...
    public void run() {
      try {
        ...
        System.out.println("Start inserting!!!!!!!!");
        long start = System.currentTimeMillis();
        pt.Batch_Insert(itemStart * 100000, 100000, 100000);
        long end = System.currentTimeMillis();
